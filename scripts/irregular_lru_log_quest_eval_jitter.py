...
hbm_capacity = 32  # sweep by editing per run
...
def run_log(prefetch_k: int, csv_path: str):
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for t in range(T):
            ...
            local_pages, sparse_pages, needed = gen_needed_pages(t, q_step)
...
def main():
    ...
    for k in [0, 4, 8]:
        out = f"results/quest_eval_jitter_step_log_prefetch{k}.csv"
        print(f"[run] jitter prefetch_k={k} -> {out}")
        run_log(k, out)
    print("[done] jitter logs written.")
